...
public class Backend implements BackendInterface<String, Double> {
	...
	/**
	 * Placeholder for findShortestPathWithMidpoint
	 */
	@Override
	public ShortestPathInterface<String, Double> findShortestPathWithMidpoint(String startBuilding,
			String midpointBuilding, String destinationBuilding) throws NoSuchElementException {
		
		// check if valid start end and midpoint
		List<String> shortestPath1 = graph.shortestPathData(startBuilding, midpointBuilding);
		List<String> shortestPath2 = graph.shortestPathData(midpointBuilding, destinationBuilding);

		//Merge Shortest Paths
		
		return new ShortestPath(null, null,null);
	}
